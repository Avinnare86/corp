<?php
/** @var array $picks year camp stage balance planned carriedOut canBook deptLocked approvedRows myRequests csrf */
$canBook = !empty($canBook);        // из контроллера: этап booking И отдел/организация не заблокированы
$deptLocked = !empty($deptLocked);
...
$cyTabLabel = function (string $cyStage): string {
    if ($cyStage === 'booking') { return 'самозапись'; }
    if ($cyStage === 'signing' || $cyStage === 'closed') { return 'изменение графика'; }
    return \App\Services\VacationCampaignService::STAGES[$cyStage] ?? $cyStage;
};
?>
<h1><?= e($title) ?></h1>
...
<?php if ($campaignYears): ?>
<div class="tabs">
    <?php foreach ($campaignYears as $cy): $cyYear = (int) $cy['year']; $cyActive = $cyYear === (int) $year; ?>
        <a class="tab <?= $cyActive ? 'active' : '' ?>" href="/vacation-campaign/booking?year=<?= $cyYear ?>">
            <?= $cyYear ?> год <span class="muted" style="font-size:.72rem;font-weight:400">(<?= e($cyTabLabel((string) $cy['stage'])) ?>)</span>
        </a>
    <?php endforeach; ?>
</div>
<?php endif; ?>
...
<section class="panel" style="border-left:4px solid #2a7">
    <h2 style="margin-top:0">Утверждённые даты (в графике на <?= (int) $year ?>)</h2>
    <table class="table tbl-cards">
        <thead><tr><th>Период</th><th>Дней</th></tr></thead>
        <tbody>
        <?php foreach ($approvedRows as $ar): ?>
            <tr>
                <td data-label="Период"><?= e(date('d.m.Y', strtotime($ar['start_date']))) ?> — <?= e(date('d.m.Y', strtotime($ar['end_date']))) ?></td>
                <td data-label="Дней"><?= (int) $ar['days'] ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    <p class="muted" style="font-size:.85rem;margin:6px 0 0">График отпусков на <?= (int) $year ?> год утверждён директором. Это ваши официальные даты.
        Изменить их можно заявкой на изменение графика (ниже).</p>
</section>
<?php endif; ?>

<?php if (!$camp): ?>
    <p class="tag warn">Кампания на <?= (int) $year ?> ещё не открыта — предложения по отпускам вводить нельзя.</p>
<?php elseif (in_array($stage, ['balances', 'blackouts'], true)): ?>
    <p class="tag warn">Кампания на <?= (int) $year ?> открыта (этап «<?= e(\App\Services\VacationCampaignService::STAGES[$stage] ?? $stage) ?>») — приём дат ещё не начался.
        Загляните сюда позже, когда откроется самозапись.</p>
<?php elseif ($canBook): ?>
<section class="panel">
...
<?php elseif ($deptLocked && $stage === 'booking'): ?>
    <p class="tag warn">Изменения по вашему отделу заблокированы начальником — идёт правка и согласование графика.
        После утверждения графика директором вы сможете подать заявку на изменение.</p>
<?php else: ?>
