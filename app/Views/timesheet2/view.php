<?php
/** Табель учёта использования рабочего времени — единая форма по ОКУД 0504421 (Приказ Минфина № 52н)
 *  для обоих видов: обычный (5/2, строка кодов) и сменный (2/2, код + часы, дробь «Я/Н» «4/8»).
 *  Строка «часы» выводится только если в табеле есть часы. Лист А4 со штампом ЭП — печать → PDF. */
...
// Есть ли в табеле часы (сменный 2/2) — тогда выводим вторую подстроку «часы».
$hasHours = false;
foreach ($rows as $r) { foreach (($r['cells_arr'] ?? []) as $cell) { if (($cell['h'] ?? '') !== '') { $hasHours = true; break 2; } } }

// «Данные для начисления заработной платы»: свод по кодам (дни/часы). Дробные «Я/Н 4/8» раскладываем
// на составляющие (Я — дневные часы, Н — ночные); день засчитывается один раз основному коду.
$pay = [];
$add = function (string $code, float $hours, bool $day) use (&$pay) {
...
$totalCols = count($dates) + ($hasHours ? 4 : 3);
?>
<!DOCTYPE html>
...
<body>
<div class="toolbar">
    <a href="/timesheet2?month=<?= e($month) ?>&half=<?= e((string)$half) ?>">← К табелям</a>
...
<div class="sheet">
    ...
    <p class="hd">Структурное подразделение <span class="u"><?= e($t['dept_name'] ?: 'вся организация') ?></span></p>
    ...
    <table class="tb">
        ...
        <tr>
            ...
            <?php if ($hasHours): ?><th style="width:30px"></th><?php endif; ?>
            <?php foreach ($dates as $d): ?><th class="dn"><?= (int)substr($d,8,2) ?></th><?php endforeach; ?>
            <th style="width:70px">Отработано <?= e($halfLabel) ?> (<?= $hasHours ? 'дни/часы' : 'дни' ?>)</th>
        </tr>
        </thead>
        <tbody>
        <?php $n = 0; foreach ($rows as $r): $n++; $cells = $r['cells_arr'] ?? []; ?>
        <tr>
            <td<?= $hasHours ? ' rowspan="2"' : '' ?>><?= $n ?></td>
            <td class="name"<?= $hasHours ? ' rowspan="2"' : '' ?>><?= e($r['full_name']) ?><br><i style="font-size:7.5pt"><?= e($r['position']) ?></i></td>
            <?php if ($hasHours): ?><td class="cap">код</td><?php endif; ?>
            <?php foreach ($dates as $i => $d): ?><td class="code"><?= e($cells[$i]['c'] ?? '') ?></td><?php endforeach; ?>
            <td<?= $hasHours ? ' rowspan="2"' : '' ?>><b><?= (int)$r['days'] ?></b><?= $hasHours ? ' / ' . e($fmt($r['hours'])) : '' ?></td>
        </tr>
        <?php if ($hasHours): ?>
        <tr>
        ...
        <?php endif; ?>
        <?php endforeach; ?>
        <?php if (!$rows): ?><tr><td colspan="<?= $totalCols ?>" class="name" style="color:#777">Нет сотрудников в табеле.</td></tr><?php endif; ?>
        </tbody>
    </table>
    ...
    <table class="pay">
        <thead><tr><th>Код вида оплаты</th><th>Вид использования рабочего времени</th><th>Дни</th><?php if ($hasHours): ?><th>Часы</th><?php endif; ?></tr></thead>
        <tbody>
        <?php foreach ($pay as $code => $agg): ?>
            <tr>
                ...
                <?php if ($hasHours): ?><td style="text-align:center"><?= $agg['hours'] > 0 ? e($fmt($agg['hours'])) : '—' ?></td><?php endif; ?>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <p class="legend"><strong>Условные обозначения:</strong><br>
        ...
        <?php endforeach; ?>
        <?php if ($hasHours): ?><em>Я/Н + «дн/ночь»</em> — рабочий день с дневными и ночными часами (дробью, напр. «4/8»).<?php endif; ?>
    </p>
