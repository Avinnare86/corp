...
<section class="panel">
    ...
    <form method="post" action="/admin/operations" class="grid-form">
        ...
        <label>Этап визы
            <select name="stage"><option value="0">— не этап —</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select>
        </label>
            ...
            <tr>
                <td colspan="4">
                    <form method="post" action="/admin/operations" class="row-form">
                        ...
                        <label>этап
                            <select name="stage"><?php foreach ([0=>'—',1=>'1',2=>'2',3=>'3'] as $sv=>$sl): ?><option value="<?= $sv ?>" <?= (int)($o['stage']??0)===$sv?'selected':'' ?>><?= $sl ?></option><?php endforeach; ?></select>
                        </label>
